...
if __name__ == "__main__":
    path = os.path.join("./", "test_trajectory.vla")

    # Create a trajectory
    traj = robodm.Trajectory(path=path, mode="w", video_codec="libaom-av1")

    # Add some data
    for i in range(10):
        cur = np.random.randint(0, 255, (224, 224, 3), dtype=np.uint8)
        traj.add(
            "observation/image",
            prev if i % 2 != 0 else cur,
        )
        prev = cur
